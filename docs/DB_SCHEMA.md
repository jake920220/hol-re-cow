...
1. 프론트엔드 브라우저 코드에서는 DB에 직접 통신하지 않는다.
2. DB 조회/변경은 Route Handler, Server Action, 서버 전용 모듈 등 서버 경계 안에서만 실행한다.
3. Client Component에는 service role key, 관리자 키, 결제 비밀 키, 내부 토큰을 절대 포함하지 않는다.
4. 결제 기능이 붙어도 결제 생성/승인/검증/웹훅 처리는 모두 서버 측에서만 수행한다.
5. 브라우저에는 사용자가 봐도 안전한 최소 정보만 내려보내고, 민감 데이터는 서버에서 필터링한다.
